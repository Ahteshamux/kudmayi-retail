...
export default async function ProductPage({
  params,
  searchParams,
}: PageProps<"/admin/catalog/product/[id]">) {
  ...
  const backHref = `/admin/catalog/category/${product.category}`;
        ...
        {isEditing ? (
          <>
            ...
            <ProductForm
              ...
              product={product}
              cancelHref={`/admin/catalog/product/${product.id}`}
          ...
          <div className="grid gap-10 md:grid-cols-2 md:gap-14">
            ...
            <div>
              ...
              <div className="border-line mt-10 flex flex-wrap items-center gap-3 border-t pt-8">
                <Link
                  href={`/admin/catalog/product/${product.id}?edit=1`}
                  className="u-btn u-caps"
                >
                  Edit
                </Link>
                <Link
                  href={`/admin/products/new?fromRetail=${product.id}`}
                  className="u-btn-ghost u-caps"
                >
                  Publish to Storefront &rarr;
                </Link>
                <DeleteProductButton id={product.id} name={product.name} />
              </div>
              <p className="text-muted mt-3 text-xs">
                Sends this piece&rsquo;s name, colour, category, and photo to a new
                storefront listing — this Retail entry is untouched, and the two
