...
function revalidateFor(category: CategorySlug, id?: string) {
  ...
  revalidatePath(`/admin/catalog/category/${category}`);
  if (id) revalidatePath(`/admin/catalog/product/${id}`);
}
...
export async function createProduct(
  ...
  redirect(`/admin/catalog/product/${data.id}`);
}
...
export async function updateProduct(
  ...
  if (existing && existing.category !== parsed.category) {
    revalidatePath(`/admin/catalog/category/${existing.category}`);
  }
...
export async function deleteProduct(formData: FormData): Promise<void> {
  ...
  redirect(
    existing?.category
      ? `/admin/catalog/category/${existing.category}`
      : "/admin/catalog",
  );
}
